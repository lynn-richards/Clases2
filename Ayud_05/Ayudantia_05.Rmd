...
```{r }
api_key <- "AIzaSyAvGk7QD5cyhpT6T2Fz020c1deUfr1nhvU"
set_key(key = api_key)
google_keys()
```
...
restaurantes_60 <- cbind(restaurantes_60,Biblioteca = Biblioteca$formatted_address,lat_B = Biblioteca$geometry.location.lat,lng_B = Biblioteca$geometry.location.lng)
```
...
#Se diferencia porque uno esta calculada en metros
```
...
leaflet() %>% 
  ...
  addAwesomeMarkers(ratings[4], lat = ratings$lat[4], lng = ratings$lng[4], popup = ratings$name[4], icon = color_r_2)
```

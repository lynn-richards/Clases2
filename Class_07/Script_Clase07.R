...
comunas_rm<- st_sf(comunas_rm) #Lo vuelve a sf, ya que es data.frame
comunas_rm<-as_Spatial(comunas_rm)

library(spdep)

nbs<-poly2nb(comunas_rm,queen = T)  #Aqui utiliza el de neibours

w_rm<-nb2listw(nbs,style = "W") #lista de vecinos a una matriz w

plot(comunas_rm)
plot(nbs,coordinates(comunas_rm),add=T,col='blue',pch=".")

comunas_rm$Confirmados_2020.04.17_sl<-lag.listw(w_rm,comunas_rm$Confirmados_2020.04.17)  #como tener los vaores de mi vecinos rezago espacial cual es el valor del rezago que estoy haciendo, en este caso voy a ver el rezago espacial de la variables w_rm, en este caso el rezago de stgo sera el promedio de mis vecinos
#para cada valor y 
...
moran.plot(comunas_rm$Confirmados_2020.04.17,listw = w_rm,) #linea media, regresion, linea punteadas de cada variable que define sus cuadrantes, entonces high value etc
...
#loess es una regresion de cada tramo por ello cambia de pendiente
#loc un resumen espacial por cada comuna
